...
csv_files = [f for f in os.listdir('Tracking') if f.endswith('.csv')]
...
for csv_file in csv_files:
    ...
    if os.path.getsize(f'Tracking/{csv_file}') == 0:
    ...
    expiration_call = re.search('EC_(.*?)_', csv_file).group(1)

    # Read the CSV file into a DataFrame
    df = pd.read_csv(f'Tracking/{csv_file}')
